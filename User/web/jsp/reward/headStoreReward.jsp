...
<body ng-app="m" ng-controller="c" ng-init='index = "奖励管理";subIndex = "总店月奖励管理"'>
...
<script>
    var m = angular.module("m", ["nm","dateRangeModule"])
        .controller("c", function($scope, page, ajax, entity, $filter,alertService){
            $scope.totalSellMoney = 0;  //统计总裂变销售额
            $scope.totalRewardMoney = 0;  //统计总裂变奖励金额
            $scope.dateRange = {
                start: null,
                end: null,
            ...
            $scope.load = function(current, size, orderBy, asc){
                ajax.ajax("/user/redirect/trade/statics/getHeadStoreRewardList","POST",
                    {
                        userId:1,
                        current: current,
                        size: size,
            ...
            $scope.ths = [
                ...
                },
                {
                    ...
                    value: function (row) {
                        var sellMoney;
                        $scope.getChildStore(row);
                        // console.log(sellMoney);
                        sellMoney = $scope.totalSellMoney;
                        $scope.totalSellMoney = 0;
                        return sellMoney;
                    },
                    width: "10%"
                }, {
                    ...
                    value: function (row) {
                        ...
                        $scope.getChildStore(row);
                        // console.log(sellMoney);
                        rewardMoney = $scope.totalRewardMoney;
                        $scope.totalRewardMoney = 0;
                        return rewardMoney;
                    },
                    width: "10%"
                }
            ...
            // $scope.getChildStore = function(childInvitedStore){
            //     for(var j=0;j<childInvitedStore.invitedStore.length;j++){
            //         if(childInvitedStore.invitedStore.length !== 0){
            //             $scope.totalSellMoney += childInvitedStore.sellMoney;
            //             $scope.totalRewardMoney += childInvitedStore.rewardMoney;
            //             $scope.getChildStore(childInvitedStore.invitedStore);
            //         }
            //         else
            //         {
            //             $scope.totalSellMoney += childInvitedStore.sellMoney;
            ...
            //     }
            // }
            $scope.getChildStore = function(data){
                $scope.totalSellMoney += data.sellMoney;
                $scope.totalRewardMoney += data.rewardMoney;
                if(data.invitedStore.length){
                    if(data.invitedStore.length>0){
                        for(var j= 0;j<data.invitedStore.length;j++)
                            $scope.getChildStore(data.invitedStore[j]);
                    }
                }
            };
        })
